...
async function generate_organization_upcoming() {
  const response = await get_oragnization_upcoming_endpoint()
      const data = await response.json();
      if (data){
        for (let i = 0; i < data.length; i++) {
          let row = organisation_upcoming_events.insertRow(i + 1)
          let cell1 = row.insertCell(0) //Name
          let cell2 = row.insertCell(1) //Date
          let cell3 = row.insertCell(2) //Start Time
          let cell4 = row.insertCell(3) //Details
          cell1.innerHTML = data[i]["Event"]
          cell2.innerHTML = data[i]["date_time"].split(' ')[0]
          cell3.innerHTML = data[i]["date_time"].split(' ')[1]
          const link_button = document.createElement('button')
          link_button.innerText = 'More Info →'
          link_button.setAttribute("class", "more_info_button");
          link_button.onclick = function() {
          window.location.href = '../pages/event_display.php?event_id='+data[i]["event_id"];
        }
        cell1.setAttribute("class", "no_padding_vert")
        cell2.setAttribute("class", "no_padding_vert")
        cell3.setAttribute("class", "no_padding_vert")
...
async function generate_organization_past() {
  const response = await get_oragnization_past_endpoint()
      const data = await response.json();
      if (data){
        for (let i = 0; i < data.length; i++) {
          let row = organisation_past_events.insertRow(i + 1)
          let cell1 = row.insertCell(0) //Name
          let cell2 = row.insertCell(1) //Start Date
          let cell3 = row.insertCell(2) //End Date
          let cell4 = row.insertCell(3) //Details
          cell1.innerHTML = data[i]["Event"]
          cell2.innerHTML = data[i]["date_time"].split(' ')[0]
          cell3.innerHTML = data[i]["date_time"].split(' ')[0]
          const link_button = document.createElement('button')
          link_button.innerText = 'More Info →'
          link_button.setAttribute("class", "more_info_button");
          link_button.onclick = function() {
...
async function generate_organization_track() {
  const response = await get_oragnization_track_endpoint()
      const data = await response.json();
      if (data){
        for (let i = 0; i < data.length; i++) {
          let row = organisation_past_events.insertRow(i + 1)
