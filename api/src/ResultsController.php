    ...
    public function process_request(string $method)
    {
        switch ($method) {
            case "GET":
                ...
                if (isset($data["participant1"])) {
                    if ($data["participant1"]) {
                        $event_results = $this->gateway->get_user_results($data["participant1"], $data["event_id"]);
                ...
                //hämta resultat inom en viss tid
                break;
            
            case "POST":
                $data = (array) json_decode(file_get_contents("php://input"), true);
                $errors = $this->get_validation_errors($data);
                $this->gateway->add_result($data["chip_id"], $data["track_time"], $data["total_time"]);
                // if (!empty($errors)) {
            ...
            case "DELETE":
                $data = $_GET;
                // $errors = $this->get_validation_errors($data);
                $this->gateway->delete_result($data["event_id"], $data["participant1"], $data["result_id"]);
                // if (!empty($errors)) {
                //     http_response_code(422);
                //     echo json_encode(["errors" => $errors]);
                // }
